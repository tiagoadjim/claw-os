...
### 🖥️ Composio Kiosk（自助终端）
ClawX 原生嵌入了 [Composio](https://composio.dev)，作为独立页面（从侧边栏打开）。在 **设置 → Composio** 中可以设置 Composio 地址，并开启 **启动时以 Kiosk 模式打开 Composio**——此时应用会在启动时直接进入全屏 Composio 体验（同时开启开机自启动，使其在开机时运行）。通过 **前往电脑** 按钮（或按 `Esc`）即可退出 Kiosk 并返回常规界面。
