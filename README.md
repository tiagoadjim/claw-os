...
Connect [Composio](https://composio.dev) so your OpenClaw agents can use hundreds of third-party app integrations (Gmail, Slack, GitHub, …) as tools. In **Settings → Composio**, paste your Composio **API key** and the **MCP URL** generated for your account, then enable it — ClawX registers Composio as an MCP server for your chat sessions. The API key is stored locally in the app's secure store and is only ever sent as the `X-API-Key` header to your Composio MCP endpoint.
