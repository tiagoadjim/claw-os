...
<p align="center">
  <img src="src/assets/logo.svg" width="128" height="128" alt="Claw OS Logo" />
</p>

<h1 align="center">Claw OS</h1>
...
<p align="center">
  ...
  <a href="#why-claw-os">Why Claw OS</a> •
...
**Claw OS** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience—no terminal required.

Whether you're automating workflows, managing AI-powered channels, or scheduling intelligent tasks, Claw OS provides the interface you need to harness AI agents effectively.

Claw OS comes pre-configured with best-practice model providers and ships in English and Spanish. Advanced configuration remains available through **Settings → Advanced → Developer Mode**.
...
Building AI agents shouldn't require mastering the command line. Claw OS was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | Claw OS Solution |
|-----------|----------------|
...
Claw OS is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
When you target another agent with `@agent`, Claw OS switches into that agent's own conversation context directly instead of relaying through the default agent. Agent workspaces stay separate by default, and stronger isolation depends on OpenClaw sandbox settings.
...
For custom channel account IDs, Claw OS enforces OpenClaw-compatible canonical IDs (`[a-z0-9_-]`, lowercase, max 64 chars, must start with a letter/number) to prevent routing mismatches.
Claw OS now also bundles Tencent's official personal WeChat channel plugin, so you can link WeChat directly from the Channels page with an in-app QR flow.
...
Claw OS also pre-bundles full document-processing skills (`pdf`, `xlsx`, `docx`, `pptx`), deploys them automatically to the managed skills directory (default `~/.openclaw/skills`) on startup, and enables them by default on first install.
...
When you edit or switch providers, Claw OS preserves existing per-model capability metadata such as `input: ["text", "image"]`. Newly selected Custom-provider models use OpenClaw onboarding-compatible image-input inference, with unknown models defaulting to text-only.
Custom-provider model rows also receive an explicit `contextWindow` (inferred from the model family, e.g. `gpt-5.x` → 272k), and rows saved by older versions are backfilled on startup, so OpenClaw can compact long sessions before they fail with "Context overflow" errors. When you have no compaction config, Claw OS seeds `agents.defaults.compaction.mode = "safeguard"` and `reserveTokensFloor = 50000`; rows or configs you authored yourself are never modified (except a missing `reserveTokensFloor` may be backfilled).
...
When a compatible gateway rejects `/models` for non-auth reasons, Claw OS automatically falls back to a lightweight `/chat/completions` or `/responses` probe during API key validation.

### 🌙 Adaptive Theming
Light mode, dark mode, or system-synchronized themes. Claw OS adapts to your preferences automatically.

### 🚀 Startup Launch Control
In **Settings → General**, you can enable **Launch at system startup** so Claw OS starts automatically after login.

### 🧩 Composio Connectors
The dedicated **Connectors** page loads Composio's live toolkit catalog, so newly supported services appear without a Claw OS release. Add a Composio project API key once; Electron Main stores it locally, creates a Tool Router session, registers its MCP endpoint in the OpenClaw Gateway, and opens Composio-hosted authentication when you connect a service. OAuth credentials never pass through the Renderer or get written to `openclaw.json`.

### 🔔 Update Prompts
Claw OS can automatically check for new versions on startup. When an update is available, it shows an in-app prompt; downloading and installing only happen after you choose the action.
...
git clone https://github.com/ValueCell-ai/ClawX.git Claw-OS
cd Claw-OS
...
When you launch Claw OS for the first time, the **Setup Wizard** will guide you through:

1. **Language** – Choose English or Spanish
2. **Agent identity** – Name the default agent
3. **Model** – Sign in with ChatGPT to use a subscription, or configure another model later
4. **Connectors** – Optionally create the Composio session used by the live Connectors catalog
5. **Capabilities** – Choose which skills, automations, channels, and plugins to prepare
6. **Readiness** – Start the assistant engine automatically

The wizard preselects your system language when it is supported, and falls back to English otherwise.

> Note for Moonshot (Kimi): Claw OS keeps Kimi web search enabled by default.
> When Moonshot is configured, Claw OS also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

Claw OS includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, Claw OS falls back to `Proxy Server`.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- Claw OS also syncs the proxy to OpenClaw's Telegram channel config when Telegram is enabled.
- Gateway restarts preserve an existing Telegram channel proxy if Claw OS proxy is currently disabled.
...
Claw OS employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:
...
- A full ACP replay can restore recorded file activity. If replay is incomplete, Claw OS does not infer missing activity through fallback behavior.
...
- Claw OS is an Electron app, so **one app instance normally appears as multiple OS processes** (main/renderer/zygote/utility). This is expected.
...
- Clicking the window close button (`X`) hides Claw OS to tray; it does **not** fully quit the app. Use tray menu **Quit Claw OS** for complete shutdown.
...
- **Package Manager**: the pnpm version pinned in `package.json` (activate it with Corepack)
...
- uses a temporary Claw OS `userData` directory
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make Claw OS better.
...
Claw OS is built on the shoulders of excellent open-source projects:
...
### Claw OS Partner Program 🚀

We're launching the Claw OS Partner Program and looking for partners who can help introduce Claw OS to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the Claw OS team provides full technical support, customization, and integration.
...
Claw OS is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
