...
describe('agent config lifecycle', () => {
  ...
  it('seeds a default Claw OS IDENTITY.md for newly created agent workspaces', async () => {
    await writeOpenClawJson({
      agents: {
        list: [{ id: 'main', name: 'Main', default: true }],
      },
    ...
    await expect(readFile(join(testHome, '.openclaw', 'workspace-research', 'IDENTITY.md'), 'utf8')).resolves.toContain('Claw OS');
  });
});
