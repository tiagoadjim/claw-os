...
### 🖥️ Composio キオスク
ClawX は [Composio](https://composio.dev) を専用ページとしてネイティブに埋め込みます（サイドバーから開けます）。**設定 → Composio** で Composio の URL を設定し、**起動時に Composio をキオスクモードで開く** を有効化すると、アプリは起動時にそのまま全画面の Composio 体験に入ります（システム起動時の自動起動も有効になり、PC 起動時に立ち上がります）。**PCへ移動** ボタン（または `Esc`）でキオスクを抜けて通常のアプリに戻れます。
