...
### 🧩 Composio 連携
[Composio](https://composio.dev) を接続すると、OpenClaw エージェントが数百のサードパーティアプリ連携（Gmail、Slack、GitHub など）をツールとして利用できます。**設定 → Composio** で、Composio の **API キー** とアカウント用に生成された **MCP URL** を貼り付けて有効化すると、ClawX が Composio をチャットセッションの MCP サーバーとして登録します。API キーはアプリのローカルな安全なストアに保存され、Composio の MCP エンドポイントには `X-API-Key` ヘッダーとしてのみ送信されます。
